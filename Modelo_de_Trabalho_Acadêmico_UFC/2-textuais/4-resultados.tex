...
Este capítulo apresenta e discute os resultados obtidos a partir da execução da bateria de vinte e cinco casos descrita na \autoref{sec:bateria-casos}, sobre as seis formulações implementadas. Os dados aqui exibidos são os do relatório consolidado mais recente da pipeline de comparação, no qual o critério de equivalência numérica entre formulações foi fixado em $|\Delta| \le 10^{-4}$ simultaneamente sobre $V_i$, $\theta_i$, $p^g_k$, $q^g_k$ e os fluxos de ramo.
...
\IBGEtab{}{
    ...
    \Fonte{elaborado pelo autor a partir do relatório consolidado da pipeline de comparação.}
}
\end{table}
...
\Caption{\label{fig:heatmap} Mapa de calor da convergência das formulações (14)--(19) na bateria de testes}
\IBGEtab{}{
\footnotesize
...
Em segundo lugar, há seis casos em que ao menos uma das formulações \textit{hand-built} converge enquanto ao menos uma das referências falha. Em três deles \textemdash \texttt{4busfrank\_vlim}, \texttt{5busfrank\_csca} e \texttt{test\_system} \textemdash \emph{ambas} as referências falham; nos outros três \textemdash \texttt{300bus} (apenas (14), como INV), \texttt{3bus\_DCline} (apenas (15)) e \texttt{3bus\_DSHL} (apenas (14)) \textemdash apenas uma referência falha. Em todos esses casos, a presença das folgas $s^v, s^q$ no problema \textit{soft-constraint} permite que as formulações \textit{hand-built} encontrem uma solução factível na qual o controle nominal não é estritamente respeitado, mas é minimamente violado \textemdash comportamento que as formulações (14) e (15) não conseguem reproduzir, dado que tratam $V$ e $q^g$ apenas com limites caixa rígidos.
...
A \autoref{tab:divergencia-3bus} mostra quatro soluções distintas para o mesmo sistema: o FPO (14) eleva todas as tensões a 1,1\,p.u.\ na barra de geração, comportamento típico de um FPO sem custo de tensão; o FP (15) converge para o ponto operacional ``natural'' da rede; a formulação \textit{hand-built} (16) fica próxima desse ponto, mas com uma diferença em $V$ de cerca de 0,006--0,012\,p.u.\ compatível com o efeito da folga $s^v$ na minimização da Equação \eqref{eq:obj-16}; e, finalmente, as formulações (17)--(19), que têm CSCA ativo, deslocam o ponto de operação para tensões consistentemente mais baixas \textemdash isso é o efeito do controle de \textit{shunt} ajustando $b^{sh}_s$ para minimizar o desvio do \textit{setpoint} de $V$ na barra de geração, que neste caso é 1,007\,p.u.\ em vez do nominal 1,03 ou 1,1. A \autoref{fig:perfil-tensao} apresenta esses valores graficamente.
