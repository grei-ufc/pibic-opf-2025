...
\subtitle{Uma progressão incremental F0 $\rightarrow$ F5 com validação sobre redes pedagógicas e um cenário integral do SIN}
...
\begin{frame}{Lacuna metodológica}
    ...
    \vspace{0.4cm}
    \successbox{\textbf{Estratégia:} desenvolver desde a concepção, em JuMP, formulações que adicionem os controles ANAREDE um a um, comparáveis às referências PowerModels sobre o mesmo conjunto de casos de teste.}
\end{frame}
...
\begin{frame}{Objetivos}
    ...
    \begin{exampleblock}{Objetivos específicos}
        \begin{enumerate}
            ...
            \item Usar \textit{PWF.jl} para leitura de arquivos ANAREDE com as estruturas de controle.
            \item Implementar \emph{duas} formulações de referência via PowerModels.
            \item Implementar \emph{quatro} formulações desenvolvidas manualmente acrescentando QLIM+VLIM, CSCA, CTAP e DERA.
            \item Submeter as seis formulações a um conjunto de \textbf{27 casos de teste}, do 3 barras ao SIN.
        \end{enumerate}
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Fluxo de Potência Ótimo e relaxações}
    ...
    \vspace{-0.1cm}
    \begin{equation*}
        \min_{V,\theta,p^g,q^g} \sum_{k \in \mathcal{G}} c_k(p^g_k) \quad \text{s.a.} \quad \text{balanço + limites operacionais}
    \end{equation*}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.48\textwidth]{relaxacao_aproximacao.png}
    \end{figure}
    \vspace{-0.3cm}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{block}{QLIM + VLIM}
                Lógica de \emph{type-switching} PV $\leftrightarrow$ PQ. Implementada por \example{folgas penalizadas}: $s^v_i$ no \textit{setpoint} de tensão (QLIM) e $s^q_l$ na demanda reativa (VLIM).
            \end{block}
            \begin{block}{CSCA}
        ...
        \begin{column}{0.5\textwidth}
            \begin{block}{CTAP}
                \textit{Tap} do transformador em carga $\rightarrow$ razão $t_{ij}$ vira \emph{variável contínua}; reaproveita as folgas de tensão de F3 (não cria folga nova).
            \end{block}
            \begin{block}{DERA}
...
\begin{frame}{Fluxo de execução}
    ...
    \vspace{0.1cm}
    \begin{enumerate}
        \item Leitura PWF com \texttt{add\_control\_data=true} (estruturas de controle);
        \item Adequação da topologia: maior componente conexa + barra \textit{slack} única;
...
\begin{frame}{A progressão F0 $\rightarrow$ F5}
    ...
    \begin{tabular}{cllll}
        ...
        \hline
        \example{F0} & PowerModels & FPO AC          & referência de FPO \\
        \example{F1} & PowerModels & FP AC           & referência de FP \\
        F2 & JuMP (manual) & FP controlado & QLIM + VLIM \\
        F3 & JuMP (manual) & FP controlado & + CSCA \\
        F4 & JuMP (manual) & FP controlado & + CTAP \\
        F5 & JuMP (manual) & FP controlado & + DERA \\
        \hline
    \end{tabular}
    \end{center}
...
\begin{frame}{Como funcionam as folgas penalizadas}
    ...
    \begin{itemize}
        \item Variáveis primárias: $V_i, \theta_i, p^g_k, q^g_k$ com limites operacionais do PWF.
        \item Folgas: $s^v_i$ em cada barra PV (QLIM), $s^q_l$ em cada carga (VLIM).
...
%% ---------------------------------------------------------------------------
\begin{frame}{Conjunto de casos de teste -- 27 casos}
    \begin{itemize}
        ...
        \item \textbf{Cenário integral do SIN:} \texttt{CASO\_VER\_MAXDIU} (13\,338 barras, PAR/PEL 2027--2031, Verão Máx.\ Diurna).
        \item \textbf{Casos de estresse:} \texttt{test\_defaults}, \texttt{test\_line\_shunt}, \texttt{test\_system}.
    \end{itemize}
...
\begin{frame}{Convergência global -- formulações manuais ampliam a faixa}
    \footnotesize
    \begin{center}
    ...
    \vspace{0.2cm}
    \successbox{\textbf{F2--F5 convergem em 23 casos}, vs.\ 18 de F1 e 15 de F0 (conjunto completo de casos).}
\end{frame}
...
\begin{frame}{Análise barra a barra -- \textit{clusters} de equivalência}
    ...
    \vspace{0.2cm}
    \begin{itemize}
        \item \example{F0}: FPO empurra $V \to 1{,}1$\,p.u.\ (limite operacional).
...
\begin{frame}{O caso SIN integral (\texttt{CASO\_VER\_MAXDIU})}
    \begin{alertblock}{Resultado negativo, mas relevante}
        \emph{Nenhuma} das seis formulações converge dentro do orçamento (\texttt{max\_iter=3000} e tempo limite de execução):
        \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Causa provável: fidelidade do modelo NLP reconstruído (HVDC, controles remotos, múltiplas referências) + condicionamento com $\rho = 10^6$ nessa escala.
...
\begin{frame}{Verificação auxiliar FDC: o mesmo SIN, em DC, converge}
    \begin{exampleblock}{FDC -- fluxo de potência DC (\texttt{solve\_dc\_pf}) sobre \texttt{CASO\_VER\_MAXDIU}}
        Mesmo fluxo de execução, mesmo arquivo, mesma máquina: encerra em \example{\texttt{LOCALLY\_SOLVED}} em poucas iterações.
    \end{exampleblock}
...
\begin{frame}{Síntese das contribuições}
    \begin{exampleblock}{1. Modelo de fluxo de potência com variáveis de controle}
        Formulação de um fluxo de potência AC em JuMP que incorpora as ações de controle do ANAREDE --- limites de reativo (QLIM) e de tensão (VLIM), shunts chaveáveis (CSCA), taps de transformador (CTAP) e corte de carga (DERA) --- como variáveis de decisão e restrições, resolvido como um único problema de otimização.
    \end{exampleblock}

    \begin{exampleblock}{2. Faixa de convergência ampliada}
...
\begin{frame}{Limitações e trabalhos futuros}
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{exampleblock}{Trabalhos futuros}
                \begin{itemize}
                    \item Viabilizar SIN: verificação de resíduos, auditoria da tradução PWF$\to$JuMP e aumento gradual das penalidades $\rho$.
                    \item Discretizar CSCA/CTAP $\rightarrow$ MINLP (Juniper/Alpine).
                    \item Acoplar F2--F5 ao \textit{ControlPowerFlow.jl} como referência de validação.
                \end{itemize}
            \end{exampleblock}
        \end{column}
    \end{columns}
    ...
    \vspace{0.4cm}
    \simplebox{\textbf{Código:} formulações F0--F5 disponíveis no repositório interno do trabalho.}
\end{frame}
