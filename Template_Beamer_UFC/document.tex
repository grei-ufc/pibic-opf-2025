...
% Título e subtítulo
\title[Fluxo de Potência com Controles ANAREDE em Julia]{\Large\textbf{Formulações de Fluxo de Potência com Ações de Controle ANAREDE em JuMP/Julia}}
...
\institute[EE/UFC]{
    \normalsize{\email{gabrielrufinomontenegro@gmail.com}}
    \newline
    ...
    \begin{itemize}
        \item Operação do SIN depende de \example{ações de controle} que vão além do FPO acadêmico:
            \begin{itemize}
                ...
                \item \textbf{DERA} -- agregadores de geração distribuída
            \end{itemize}
        \item Esses controles são parte da prática do \emph{ANAREDE} (CEPEL) e do formato PWF.
...
\begin{frame}{Lacuna metodológica}
    \begin{columns}
        ...
        \end{column}
        \begin{column}{0.5\textwidth}
            \begin{alertblock}{ControlPowerFlow.jl}
                \begin{itemize}
...
\begin{frame}{Fluxo de Potência AC}
    ...
    \vspace{0.2cm}
    Sistema algébrico \emph{não linear e não convexo} -- resolvido tipicamente por Newton-Raphson.
\end{frame}
...
\begin{frame}{Fluxo de Potência Ótimo e relaxações}
    ...
    \vspace{0.1cm}
    \alertbox{O objetivo SOC é uma \emph{cota inferior provável} do AC -- \textit{gap} $\approx 18\%$ neste caso.}
\end{frame}
...
\begin{frame}{Ações de controle do ANAREDE}
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{block}{CTAP}
                \textit{Tap} OLTC $\rightarrow$ razão $t_{ij}$ vira \emph{variável contínua}, com folga $s^t$ no \textit{setpoint} de tensão.
            \end{block}
            \begin{block}{DERA}
                Agregador de GD: injeções $(p^{der}, q^{der})$ entram como \emph{termos constantes} no balanço da barra hospedeira.
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Pipeline de execução}
    \begin{exampleblock}{Ambiente Julia}
        \texttt{PWF.jl} (parser ANAREDE) $+$ \texttt{PowerModels.jl} (referência) $+$ \texttt{JuMP.jl} (modelagem) $+$ \texttt{Ipopt.jl} (\textit{solver} NLP).
    \end{exampleblock}
...
\begin{frame}{Bateria de testes -- 27 casos}
    \begin{itemize}
        ...
        \item \textbf{\textit{Snapshot} integral do SIN:} \texttt{01 MAXIMA NOTURNA\_DEZ25} ($\approx$ 6\,000 barras).
...
\begin{frame}{Convergência global -- \emph{hand-built} ganham faixa}
    ...
    \begin{tabular}{lcccccc}
        ...
        \texttt{01 MAXIMA NOTURNA}     & \cellcolor{statuskil}KIL & \cellcolor{statusinf}INF & \cellcolor{statuskil}KIL & \cellcolor{statusinf}INF & \cellcolor{statuskil}KIL & \cellcolor{statusinf}INF \\
        \hline
    \end{tabular}
    \end{center}

    \vspace{0.2cm}
    \successbox{\textbf{F2--F5 convergem em 22 casos}, vs.\ 18 de F1 e 15 de F0 (bateria completa).}
\end{frame}
...
%% ---------------------------------------------------------------------------
\begin{frame}{O caso SIN integral (\texttt{01 MAXIMA NOTURNA\_DEZ25})}
    ...
    \vspace{0.2cm}
    \begin{itemize}
        \item Causa provável: ponto de partida \textit{flat} está longe demais da região factível para o método de pontos interiores escapar dentro do orçamento.
        ...
        \item Obstáculo é \emph{puramente numérico}.
    \end{itemize}
\end{frame}
...
\begin{frame}{Síntese das contribuições}
    \begin{exampleblock}{1. Faixa de convergência ampliada}
        F2--F5 convergem em \emph{22 dos 27 casos}; em 6 casos, encontram solução factível enquanto ao menos uma das referências falha.
    \end{exampleblock}
    ...
    \begin{alertblock}{3. Fronteira numérica atual}
        Nenhuma formulação converge no SIN com inicialização \textit{flat} e orçamento de 3\,000 iterações. Delimita com clareza onde a abordagem \textit{soft-constraint} pura precisa de inicialização guiada.
    \end{alertblock}
\end{frame}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{alertblock}{Limitações}
                \begin{itemize}
                    ...
                    \item DERA minimalista: injeção fixa, sem despacho próprio.
        ...
        \begin{column}{0.5\textwidth}
            \begin{exampleblock}{Trabalhos futuros}
                \begin{itemize}
                    \item Viabilizar SIN: \emph{warm-start} com DC/SOC e escalonamento de $\rho$.
...
\begin{frame}{}
    ...
    \vspace{0.2cm}
    \large{\email{gabrielrufinomontenegro@gmail.com}}
