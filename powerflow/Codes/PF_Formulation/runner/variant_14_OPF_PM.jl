# variant_14_OPF_PM.jl — adaptação do (14)OPF_PM.jl original para o runner.
# Original em ../(14)OPF_PM.jl é IMUTÁVEL; toda lógica nova vive aqui.
#
# Diferenças vs original:
#  - Caminho do PWF e diretório de saída vem de ENV (PIBIC_PWF, PIBIC_CSV_DIR)
#  - Escreve schema canônico (barras/ramos/convergencia) em vez de _PM.csv
#  - Angulos em radianos (não graus)
#  - Computa loss_q_pu como qf+qt
#  - Wrap em try/catch que grava failure row se algo falhar
...
const PIBIC_CSV_DIR = get(ENV, "PIBIC_CSV_DIR",
                          joinpath(@__DIR__, "out_default", "14"))
const PIBIC_CASE_ID = get(ENV, "PIBIC_CASE_ID", basename(PIBIC_PWF))
const PIBIC_SCRIPT  = "14"
mkpath(PIBIC_CSV_DIR)

println("[variant_14] case=$PIBIC_CASE_ID  out=$PIBIC_CSV_DIR")
...
try
    ...
    # Múltiplas barras slack → demote excedentes para PV
    ...
    if status != "LOCALLY_SOLVED"
        ...
        println("[variant_14] não convergiu (status=$status); somente convergencia.csv gravado.")
        exit(0)
    end
    ...
    # ----- df_ramos canônico (computa fluxos a partir da solução PM) -----
    df_ramos = DataFrame(
        caso = String[], script = String[],
    ...
    println("[variant_14] OK  barras=$(nrow(df_barras))  ramos=$(nrow(df_ramos))  p_loss_total=$(sum(df_ramos.loss_p_pu))")
catch e
    @warn "[variant_14] OTHER_ERROR" exception=(e, catch_backtrace())
    write_failure_row(case=PIBIC_CASE_ID, script=PIBIC_SCRIPT, out=PIBIC_CSV_DIR,
