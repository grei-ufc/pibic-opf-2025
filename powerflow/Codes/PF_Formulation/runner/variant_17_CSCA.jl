# variant_17_CSCA.jl — adaptação do (17)QLIM+VLIM+CSCA.jl original para o runner.
# Original em ../(17)QLIM+VLIM+CSCA.jl é IMUTÁVEL.
...
const PIBIC_CSV_DIR = get(ENV, "PIBIC_CSV_DIR",
                          joinpath(@__DIR__, "out_default", "17"))
const PIBIC_CASE_ID = get(ENV, "PIBIC_CASE_ID", basename(PIBIC_PWF))
const PIBIC_SCRIPT  = "17"
mkpath(PIBIC_CSV_DIR)

println("[variant_17] case=$PIBIC_CASE_ID  out=$PIBIC_CSV_DIR")
...
function resolver_fluxo_controlado(caminho_arquivo)
    ...
    # Patch de limpeza: remove chaves não-numéricas que PWF adiciona e que quebram PowerModels
    ...
    if status != "LOCALLY_SOLVED"
        ...
        println("[variant_17] não convergiu.")
        return
    end
    ...
    println("[variant_17] OK  barras=$(nrow(df_barras))  ramos=$(nrow(df_ramos))  p_loss_total=$(sum(df_ramos.loss_p_pu))")
end

try
    resolver_fluxo_controlado(PIBIC_PWF)
catch e
    @warn "[variant_17] OTHER_ERROR" exception=(e, catch_backtrace())
    write_failure_row(case=PIBIC_CASE_ID, script=PIBIC_SCRIPT, out=PIBIC_CSV_DIR,
