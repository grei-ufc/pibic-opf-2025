# variant_18_CTAP.jl — adaptação do (18)QLIM+VLIM+CSCA+CTAP.jl original para o runner.
# Original em ../(18)QLIM+VLIM+CSCA+CTAP.jl é IMUTÁVEL.
...
const PIBIC_PWF     = get(ENV, "PIBIC_PWF",
                          joinpath(@__DIR__, "..", "..", "data", "3bus.pwf"))
const PIBIC_CSV_DIR = get(ENV, "PIBIC_CSV_DIR",
                          joinpath(@__DIR__, "out_default", "18"))
const PIBIC_CASE_ID = get(ENV, "PIBIC_CASE_ID", basename(PIBIC_PWF))
const PIBIC_SCRIPT  = "18"
mkpath(PIBIC_CSV_DIR)

println("[variant_18] case=$PIBIC_CASE_ID  out=$PIBIC_CSV_DIR")
...
function resolver_fluxo_controlado(caminho_arquivo)
    ...
    if status != "LOCALLY_SOLVED"
        ...
        println("[variant_18] não convergiu.")
        return
    end
    ...
    println("[variant_18] OK  barras=$(nrow(df_barras))  ramos=$(nrow(df_ramos))  p_loss_total=$(sum(df_ramos.loss_p_pu))")
end

try
    resolver_fluxo_controlado(PIBIC_PWF)
catch e
    @warn "[variant_18] OTHER_ERROR" exception=(e, catch_backtrace())
    write_failure_row(case=PIBIC_CASE_ID, script=PIBIC_SCRIPT, out=PIBIC_CSV_DIR,
