...
set_optimizer_attribute(model, "print_level", 0) # Menos texto no console
...
# Nota: pg é variável apenas na referência. Nas outras, será fixada.
@variable(model, ref[:gen][i]["pmin"] <= pg[i in keys(ref[:gen])] <= ref[:gen][i]["pmax"])
@variable(model, ref[:gen][i]["qmin"] <= qg[i in keys(ref[:gen])] <= ref[:gen][i]["qmax"])

# --- IMPLEMENTAÇÃO MANUAL DOS CONTROLES ---

# 1. SLACK DE TENSÃO (Para simular comportamento PV/PQ do QLIM)
# Se o gerador tiver reativo suficiente, sl_v = 0 e V = V_setpoint.
# Se o reativo bater no limite, sl_v assume valor para permitir que V caia/suba.
gen_buses = unique([gen["gen_bus"] for (i,gen) in ref[:gen]])
@variable(model, sl_v[i in gen_buses], start=0.0) 

# Penalidade alta para forçar V a ficar no setpoint se possível
PENALIDADE_V = 1e6 

# =========================================================================
# 3. RESTRIÇÕES DE FIXAÇÃO (O "SEGREDO" DO FLUXO DE POTÊNCIA)
# =========================================================================
...
for (i, gen) in ref[:gen]
    ...
    # Lógica para Geração Ativa (P):
    if bus_id in slack_bus_idx
        # Na barra slack, o Pg é LIVRE (dentro dos limites) para fechar o balanço
        # A restrição já está na definição da variável pg
    else
        # Nas barras PV e PQ, o Pg é FIXO no valor do arquivo
        fix(pg[i], gen["pg"]; force=true)
    end

    # Lógica para Tensão e Reativo (O QLIM Manual):
    # A restrição é: V_calculado == V_setpoint + slack
    # O solver tentará deixar 'slack' zero por causa do custo alto na função objetivo.
    # Mas se Qg bater no limite (qmin/qmax), o solver usará a slack.
    
    v_setpoint = get(ref[:bus][bus_id], "vm", 1.0)
    
    # Restrição Suave de Tensão:
...
# =========================================================================
# (Reaproveitando sua implementação eficiente)
...
# =========================================================================
# 5. OBJETIVO: MINIMIZAR DESVIO DE TENSÃO
# =========================================================================

# Aqui está a mágica do QLIM manual.
# Minimizamos o quadrado da variável slack de tensão.
# Se o sistema consegue operar no setpoint, sl_v será 0.
# Se não, ele encontrará o menor desvio possível.
@objective(model, Min, sum(PENALIDADE_V * sl_v[i]^2 for i in keys(sl_v)))

println(">> Resolvendo Fluxo de Potência com QLIM Manual...")
optimize!(model)
...
open(joinpath(PASTA_CSV, "resultados_barras.csv"), "w") do io
...
open(joinpath(PASTA_CSV, "resultados_geracao.csv"), "w") do io
...
open(joinpath(PASTA_CSV, "resultados_linhas.csv"), "w") do io
