...
# Baseado na formulação matemática estrita do PowerModels.jl
# =========================================================================
...
# Definindo limites manuais para TODAS as barras
v_min_global = 0.95
v_max_global = 1.05
...
# =========================================================================
# 3. A LÓGICA DO FLUXO DE POTÊNCIA (Barras Slack e PV)
# =========================================================================
for (i, gen) in ref[:gen]
    bus_id = gen["gen_bus"]
    if bus_id in keys(ref[:ref_buses])
        # É barra Slack: O Pg fica livre para absorver as perdas
        if has_lower_bound(pg[i]); delete_lower_bound(pg[i]); end
        if has_upper_bound(pg[i]); delete_upper_bound(pg[i]); end
    else
        # É barra PV: O Pg é rigidamente fixado
        fix(pg[i], gen["pg"]; force=true)
    end
end

for (i, bus) in ref[:ref_buses]
    # Trava os ângulos de referência nos valores do caso base (evita conflito HVDC)
...
@variable(model, sl_d[i in keys(ref[:load])], start=0.0) # Slack do VLIM

for (i, gen) in ref[:gen]
    bus_id = gen["gen_bus"]
    vm_setpoint = ref[:bus][bus_id]["vm"] 
    # Formulação Clássica do QLIM
    @constraint(model, vm[bus_id] == vm_setpoint + sl_v[bus_id])
end

# =========================================================================
# 5. EQUAÇÕES DE FLUXO NOS RAMOS (Fidelidade 100% à imagem enviada)
# =========================================================================
...
for (l, branch) in ref[:branch]
    ...
    # Expressões Exatas da Foto (Nó de Envio - fr)
    ...
    # Expressões Exatas da Foto (Nó de Recebimento - to)
...
# =========================================================================
# 6. LEIS DE KIRCHHOFF DOS NÓS (Com injeção estrita do VLIM)
# =========================================================================
...
for (i, bus) in ref[:bus]
    ...
    # Formulação Clássica do VLIM (Corte somado à carga nominal)
